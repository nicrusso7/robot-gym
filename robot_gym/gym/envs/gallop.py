...
import numpy as np
from robot_gym.model.robots.spotmicro import mark_constants, rex_constants

from robot_gym.model.robots.spotmicro import GaitPlanner

from .. import rex_gym_env

# Radiant
from robot_gym.model.robots.spotmicro import Kinematics
...
class RobotReactiveEnv(rex_gym_env.RobotGymEnv):
    """The gym environment for Spotmicro.

  It simulates the locomotion of Spotmicro, a _quadruped robot. The state space
  include the angles, velocities and torques for all the motors and the action
  space is the desired motor angle for each motor. The reward function is based
  on how far Spotmicro walks in 1000 steps and penalizes the energy
  expenditure.
  ...
  """
    ...
    def __init__(self,
                 ...
                 terrain_id=None,
                 mark='base'):
        """Initialize Spotmicro trotting gym environment.
    ...
    Args:
      ...
      on_rack: Whether to place Spotmicro on rack. This is only used to debug
        the walk gait. In this mode, Spotmicro's base is hung midair so
    ...
    def reset(self):
        ...
        super(RobotReactiveEnv, self).reset(initial_motor_angles=self.init_pose, reset_duration=0.5)
        self.goal_reached = False
        self._stay_still = False
        self.is_terminating = False
    ...
    def _transform_action_to_motor_command(self, action):
        ...
        action = super(RobotReactiveEnv, self)._transform_action_to_motor_command(action)
        return action
    ...
    def is_fallen(self):
        """Decides whether Spotmicro is in a fallen state.

    If the roll or the pitch of the base is greater than 0.3 radians, the
    rex is considered fallen.

    Returns:
      Boolean value that indicates whether Spotmicro has fallen.
    """
