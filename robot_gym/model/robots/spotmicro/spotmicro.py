...
import re
import numpy as np
from robot_gym.model.world import terrain
from robot_gym.model.robots.spotmicro import spotmicro_mark, spotmicro_constants
from robot_gym.model.robots.spotmicro import motor
...
class Spotmicro:
    """The Spotmicro class that simulates a _quadruped robot."""
    ...
    def __init__(self,
                 ...
                 terrain_id='plane',
                 mark='base'):
        """Constructs a Spotmicro and reset it to the initial states.
        ...
        Args:
          ...
          motor_overheat_protection: Whether to shutdown the motor that has exerted
            ...
            details.
          on_rack: Whether to place the Spotmicro on rack. This is only used to debug
            the walk gait. In this mode, the Spotmicro's base is hanged midair so
            that its walk gait is clearer to visualize.
        """
        self.mark = mark
        self.num_motors = spotmicro_mark.MARK_DETAILS['motors_num'][self.mark]
        self.num_legs = 4
        self.motors_name = spotmicro_mark.MARK_DETAILS['motors_names'][self.mark]
        ...
        self.initial_pose = spotmicro_constants.INIT_POSES[pose_id]
    ...
    def Reset(self, reload_urdf=True, default_motor_angles=None, reset_time=3.0):
        """Reset the Spotmicro to its initial states.

        Args:
          reload_urdf: Whether to reload the urdf file. If not, Reset() just place
            the Spotmicro back to its starting position.
          default_motor_angles: The default motor angles. If it is None, Spotmicro
        ...
        if reload_urdf:
            if self._self_collision_enabled:
                self.quadruped = self._pybullet_client.loadURDF(
                    pybullet_data.getDataPath() + f"/assets/urdf/{spotmicro_mark.MARK_DETAILS['urdf_name'][self.mark]}",
                    init_position,
                    INIT_ORIENTATION,
        ...
        if reset_time > 0.0 and default_motor_angles is not None:
            ...
            if len(default_motor_angles) != spotmicro_mark.MARK_DETAILS['motors_num'][self.mark]:
                # extend with arm rest pose
                default_motor_angles = np.concatenate((default_motor_angles, spotmicro_constants.ARM_POSES["rest"]))
                pose = np.concatenate((pose, spotmicro_constants.ARM_POSES["rest"]))
            self.ReceiveObservation()
            for _ in range(100):
    ...
    def ResetPose(self):
        """Reset the pose of the Spotmicro."""
    ...
    def _ResetPoseForLeg(self, leg_id):
        ...
        self._pybullet_client.resetJointState(self.quadruped,
                                              ...
                                              spotmicro_constants.INIT_POSES[self.pose_id][3 * leg_id],
                                              targetVelocity=0)

        self._pybullet_client.resetJointState(self.quadruped,
                                              self._joint_name_to_id[f"motor_{leg_position}_leg"],
                                              spotmicro_constants.INIT_POSES[self.pose_id][3 * leg_id + 1],
                                              targetVelocity=0)
        self._pybullet_client.resetJointState(self.quadruped,
                                              self._joint_name_to_id[f"foot_motor_{leg_position}"],
                                              spotmicro_constants.INIT_POSES[self.pose_id][3 * leg_id + 2],
                                              targetVelocity=0)
    ...
    def _ResetArmMotors(self):
        for i in range(len(spotmicro_mark.ARM_MOTOR_NAMES)):
            self._pybullet_client.resetJointState(self.quadruped,
                                                  self._joint_name_to_id[spotmicro_mark.ARM_MOTOR_NAMES[i]],
                                                  spotmicro_constants.ARM_POSES['rest'][i],
                                                  targetVelocity=0)
            if self._accurate_motor_model_enabled or self._pd_control_enabled:
                # Disable the default motor in pybullet.
                self._pybullet_client.setJointMotorControl2(
                    bodyIndex=self.quadruped,
                    jointIndex=(self._joint_name_to_id[spotmicro_mark.ARM_MOTOR_NAMES[i]]),
                    controlMode=self._pybullet_client.VELOCITY_CONTROL,
                    targetVelocity=0,
                    force=0)

    def GetBasePosition(self):
        """Get the position of Spotmicro's base.

        Returns:
          The position of Spotmicro's base.
        """
        position, _ = (self._pybullet_client.getBasePositionAndOrientation(self.quadruped))
        return position

    def GetTrueBaseRollPitchYaw(self):
        """Get Spotmicro's base orientation in euler angle in the world frame.
    ...
    def GetTrueBaseOrientation(self):
        """Get the orientation of Spotmicro's base, represented as quaternion.

        Returns:
          The orientation of Spotmicro's base.
        """
    ...
    def GetBaseOrientation(self):
        ...
        This function mimicks the noisy sensor reading and adds latency.
        Returns:
          The orientation of Spotmicro's base polluted by noise and latency.
        """
        return self._pybullet_client.getQuaternionFromEuler(self.GetBaseRollPitchYaw())

    def GetTrueBaseRollPitchYawRate(self):
        """Get the rate of orientation change of the Spotmicro's base in euler angle.

        Returns:
          rate of (roll, pitch, yaw) change of the Spotmicro's base.
        """
    ...
    def GetBaseRollPitchYawRate(self):
        ...
        This function mimicks the noisy sensor reading and adds latency.
        Returns:
          rate of (roll, pitch, yaw) change of the Spotmicro's base polluted by noise
          and latency.
        """
        return self._AddSensorNoise(
    ...
    def ApplyAction(self, motor_commands, motor_kps=None, motor_kds=None):
        """Set the desired motor angles to the motors of the Spotmicro.
        ...
        Args:
          ...
          motor_kps: Proportional gains for the motor model. If not provided, it
            uses the default kp of the Spotmicro for all the motors.
          motor_kds: Derivative gains for the motor model. If not provided, it
            uses the default kd of the Spotmicro for all the motors.
        """
    ...
    def SetBaseMasses(self, base_mass):
        """Set the mass of Spotmicro's base.
    ...
    def SetBaseInertias(self, base_inertias):
        """Set the inertias of Spotmicro's base.
